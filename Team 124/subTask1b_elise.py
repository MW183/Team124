...
mdiff = MoveDifferential(OUTPUT_A, OUTPUT_B, EV3EducationSetTire, 5.98*25.4) 
#6*25 is slightly less that 6 inches into mm (less so drift decreases)
...
def main():
    ...
    for i in range(num_laps):
        if i == 4:
            error_margin = 2
        #forward
        mdiff.on_for_distance(SpeedRPM(40), distance_cm*10) #with straighten all 10 is *.5 to check halfway
